...
**指针系统**
- 时针：双截式剑形，前部红色锥形 + 尾部深色，带暗色中线
- 分针：双截式剑形，前部红色锥形 + 尾部深色，带暗色中线
- 秒针：蓝色极细针
- 中心：深蓝三层金属质感装饰帽
...
| `background/center_cap.png` | 18x18 | 深蓝中心装饰帽 |
| `hands/hour_hand.png` | 18x120 | 时针（前部红色+尾部深色） |
| `hands/minute_hand.png` | 14x155 | 分针（前部红色+尾部深色） |
| `hands/second_hand.png` | 6x190 | 秒针（蓝色极细） |
| `icons/ic_heart.png` | 14x14 | 心率图标（红色） |
| `icons/week/1~7.png` | 44x18 | 星期一~日（蓝色） |
| `weather/0~7.png` | 28x28 | 天气图标 (8种) |
...
```
时针 (18x120):  pivot_x=9, pivot_y=90   → 前部90px（红色），尾部30px（深色）
分针 (14x155):  pivot_x=7, pivot_y=120  → 前部120px（红色），尾部35px（深色）
秒针 (6x190):   pivot_x=3, pivot_y=160  → 尖端长度160px，尾部30px
```
...
```python
BLUE = (50, 85, 155, 255)         # 修改数字/刻度蓝色
RED = (200, 40, 50, 255)          # 修改指针前部红色
DARK = (25, 30, 45, 255)          # 修改指针尾部深色
```
