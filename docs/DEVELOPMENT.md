...
表盘整体采用**经典商务风格**设计，主要视觉元素包括：

**表盘底色与边框**
- 最外圈：深灰色边框 → 金色细环 → 浅金色过渡环
- 表面底色：温润的米白色 (#F8F5F0)，区别于冷白色，更具质感

**时标与刻度**
- 12个罗马数字 (I ~ XII) 采用衬线体，深藏青色 (#1A1A2E)
- 整点位置：粗长金色刻度线 (3px宽)
- 分钟位置：细短浅灰色刻度线 (1px宽)
- 内圈金色细装饰环，四个方位点缀菱形装饰
...
**信息面板 (中部)**
- 上方：天气图标 + 温度 + 天气文字
- 中部左面板：星期 + 月/日 (公历)，浅奶油色底 + 金色边框
- 中部右面板：农历日期 + 时辰，浅奶油色底 + 金色边框
- 下方居中：数字时间 (HH:MM)
...
- 罗马数字：DejaVu Serif Bold / Liberation Serif Bold，32px
- 数字时间：DejaVu Sans，34px
- 数据文字：系统默认字体，16-18px
- 中文文字：文泉驿正黑 / Noto Sans CJK，16-18px

### 2.4 与参考图的差异化

相比参考的三款商务表盘（均使用阿拉伯数字、黑白/蓝白色调、分散式信息布局），本设计在以下方面进行了差异化：

1. **时标形式**：使用罗马数字替代阿拉伯数字
2. **色调选择**：金色暖色调替代冷色黑白/蓝白
3. **底色处理**：米白色替代纯白，更有质感温度
4. **信息布局**：对称双面板替代散落分布
5. **数据展示**：紧凑双行替代环绕式子表盘
6. **指针设计**：带金色装饰线的锥形指针
...
| 文件路径 | 尺寸 | 说明 |
|---------|------|------|
| `background/bg_main.png` | 454x454 | 表盘底色与外圈 |
| `background/tick_ring.png` | 454x454 | 分钟/整点刻度 |
| `background/roman_numerals.png` | 454x454 | 罗马数字 I~XII |
| `background/inner_ring.png` | 454x454 | 内圈装饰环 |
| `background/center_cap.png` | 24x24 | 中心装饰帽 |
| `hands/hour_hand.png` | 28x140 | 时针 |
| `hands/minute_hand.png` | 20x180 | 分针 |
| `hands/second_hand.png` | 8x200 | 秒针 |
| `icons/ic_steps.png` | 20x20 | 步数图标 |
| `icons/ic_calories.png` | 20x20 | 卡路里图标 |
| `icons/ic_distance.png` | 20x20 | 距离图标 |
| `icons/ic_heart.png` | 18x18 | 心率图标 |
| `icons/ic_battery.png` | 18x18 | 电池图标 |
| `icons/separator.png` | 2x16 | 数据分隔线 |
| `icons/week/1~7.png` | 50x22 | 星期一~日 |
| `weather/0~7.png` | 40x40 | 天气图标 (8种) |
...
```
时针 (28x140):  pivot_x=14, pivot_y=115  → 尖端长度 115px，尾部 25px
分针 (20x180):  pivot_x=10, pivot_y=150  → 尖端长度 150px，尾部 30px
秒针 (8x200):   pivot_x=4,  pivot_y=170  → 尖端长度 170px，尾部 30px
```
